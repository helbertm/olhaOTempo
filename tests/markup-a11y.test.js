...
test("markup principal preserva semantica critica de acessibilidade", () => {
  ...
  assert.match(html, /id="quick-start-button"[\s\S]*aria-label="Abrir modo apresentação"/);
  assert.match(html, /class="app-beta-badge">BETA</);
  assert.match(html, /class="app-beta-note">Versão beta\./);
});
